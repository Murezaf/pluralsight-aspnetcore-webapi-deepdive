...
using CourseLibrary.Infrastructure.Helpers;
using Microsoft.EntityFrameworkCore;

namespace CourseLibrary.Infrastructure.RepositoryImplementations;
...
#pragma warning restore CS8603 // Possible null reference return.
    ...
    public async Task<PagedList<Author>> GetAuthorsAsync(AuthorRecourseParameters authorRecourseParameters)
    {
        ...
        var items = await collection
            .Skip((authorRecourseParameters.PageNumber - 1) * authorRecourseParameters.PageSize)
            .Take(authorRecourseParameters.PageSize)
            .ToListAsync();

        return new PagedList<Author>(items, totalCount, authorRecourseParameters.PageNumber, authorRecourseParameters.PageSize);
    }
}